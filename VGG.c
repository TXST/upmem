...
void init_memory() {
    ...
    for (i = 0; i < 3; i++) {
        image[i] = malloc(IMG_SIZE * sizeof(float*));
        for (j = 0; j < IMG_SIZE; j++) {
            image[i][j] = malloc(IMG_SIZE * sizeof(float));
        }
    }
...
void convolution_3_x_3(float **matrix, float **kernel, float **out, int size) {
    int i, j;
    float sum;
    float zeropad[IMG_SIZE + 2][IMG_SIZE + 2] = {0.0 };
...
void check_output(float ***in,float ***out,int level){
...
#pragma omp parallel for schedule(dynamic,1) num_threads(numthreads)
    ...
    printf("\ninput:\n");
    for(int z = 0;z < 49;z ++){
        printf("%f ",mem_block1_dense[z]);
    }
    printf("\nweight:\n");
    for(int z = 0;z < 3;z ++){
        for(int t = 0;t < 3;t ++){
            printf("%f ",wd[level][z][t]);
        }
    }
    printf("\noutput:\n");
    for(int z = 0;z < 49;z ++){
        printf("%f ",mem_block2_dense[z]);
    }
